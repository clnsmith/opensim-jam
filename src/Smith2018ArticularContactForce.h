...
/**
 ...
 meshes representing cartilage, mensici, or artfical components [1]. The 
 formulation of the contact model has previously been called an elastic 
 foundation model [2] or discrete element analysis []. In this implementation, the 
 ...
 force approach is applied. Smith et al, CMBBE I&V, 2018 [1] introduced a method 
 ...
 casting mesh towards the target mesh. Thus, a pressure map is only generated 
 for the casting mesh. A force vector is computed for each triangle in the 
 casting_mesh F = -normal*area*pressure, and the resultant force of the 
 summation of all triangle forces is applied to the casting_mesh in the 
 computeForce() function. The equal and opposite force is applied to the 
 target_mesh. Because it can still be useful to visualize the pressure maps 
 for the target_mesh, and in most situations the resultant force from the 
 target_mesh are very close to the mirrored casting_mesh resultant force, there
 is a ModelingOption named "flip_meshes" that will cause the ray casting to 
 also be performed from the target_mesh to calculate triangle proximity and 
 pressure values. Note the applied contact force in this case is still only 
 that calculated for the casting_mesh. 
 ...
 are those proposed in Bei and Fregly, Med Eng Phys, 2004 [2]:
/
*
 Linear: 
 \f[
 ...
 the two cartilage layers are lumped together, necessitating that a constant
 thickness, elastic modulus, and poissons ratio is assumed for each contacting
 triangle pair. Thus the total overlap depth is split equally between each
 triangle contact pair. As cartilage-cartilage contact often involves 
 articulations between cartilage surfaces with varying thickness and material 
 properties, the Bei and Fregly approach was extended to accomodate variable 
 properties. The use_lumped_contact_model property controls whether the 
 constant property or variable property formulation is used.
 
 The variable property formulation is described in Zevenbergen et al, 
 PLOS One, 2018 [3]. Here, the following system of four equations must be
 ...
 target_mesh multiple times, or at in feasible contact locations. An example
 ...
 This component has some outputs such as proximity, pressure, and potential
 energy that return a SimTK::Vector with a value corresponding to each 
 triangle face in the respective target or casting mesh. There are also 
 "summary" outputs that return values associated with the entire mesh such as
 contact area, mean/max proximity/pressure, center of proximity/pressure etc. 
 Finally, there are regional outputs which return a 1 x 6 SimTK::Vector. Here,
 the entries in the vector reflect the summary metrics in a subspace of the 
 mesh. The entries in the 1 x 6 SimTK::Vector correspond to the subset of 
 ...
 enable comparisons of the loading in the medial and lateral compartments. 
    ...
    [3] Zevenbergen, L., Smith, C. R., Van Rossom, S., Thelen, D. G., Famaey,
